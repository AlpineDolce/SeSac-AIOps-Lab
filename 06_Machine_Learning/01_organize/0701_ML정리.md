...
1.  **인덱스 정렬 (Alignment)**: 연산을 수행하기 전에 Pandas는 연산에 참여하는 모든 객체의 행(row) 및 열(column) 인덱스를 자동으로 정렬합니다. 즉, 동일한 레이블을 가진 요소끼리 연산이 이루어집니다. 이는 데이터의 무결성을 유지하면서 유연한 연산을 가능하게 합니다.
...
2.  **NaN (결측치) 처리**: 연산 대상 Series 중 한쪽에만 존재하는 인덱스가 있을 경우, 해당 위치의 결과는 `NaN`이 됩니다. 이는 데이터의 불완전성을 나타냅니다.
    ...
    # dtype: float64
    ```
    `eng` 인덱스는 `series_nan1`에 없으므로, `eng`에 해당하는 연산은 `NaN`으로 처리됩니다. 마찬가지로 `mat` 인덱스는 `series_nan2`에 없으므로 `NaN`이 됩니다. 이는 Pandas가 누락된 데이터를 자동으로 `NaN`으로 채워 연산을 수행하기 때문입니다.
...
2.  **DataFrame 간의 연산**: 두 DataFrame 간의 연산은 행 인덱스와 열 인덱스 모두를 기준으로 정렬된 후 수행됩니다. 공통되지 않은 인덱스/컬럼에 대해서는 `NaN`이 발생합니다.
    ...
    # z  NaN  26.0   NaN
    ```
    위 결과에서 `A` 컬럼은 `df_b`에 없고, `C` 컬럼은 `df_a`에 없으므로 해당 컬럼들은 `NaN`이 됩니다. 또한, `x` 행은 `df_b`에 없고, `w` 행은 `df_a`에 없으므로 해당 행들도 `NaN`이 됩니다. 오직 공통된 인덱스와 컬럼(`y`, `z` 행의 `B` 컬럼)에 대해서만 연산이 수행됩니다. 이는 데이터 병합 및 분석 시 발생할 수 있는 결측치를 이해하는 데 중요합니다.
