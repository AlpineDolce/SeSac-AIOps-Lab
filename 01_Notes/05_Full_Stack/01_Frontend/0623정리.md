...
- [1. HTML 개요 및 기본 구조](#1-html-개요-및-기본-구조)
  ...
  - [1.2. 문서 기본 구조 요소](#12-문서-기본-구조-요소)
    ...
    - [1.2.3. `<head>` 태그: 메타데이터 및 외부 리소스](#123-head-태그-메타데이터-및-외부-리소스)
      ...
      - [1.2.3.3. 성능 최적화 관련 <link> 태그](#1233-성능-최적화-관련-link-태그)
...
- [4. 웹 표준, 보안 및 실무 고급 기법](#4-웹-표준-보안-및-실무-고급-기법)
  ...
  - [4.3. HTML과 보안 (HTML and Security)](#43-html과-보안-html-and-security)
    ...
    - [4.3.3. 콘텐츠 보안 정책 (CSP)](#433-콘텐츠-보안-정책-csp)
...
- **입력 요소 (`<input>`, `<select>`, `<textarea>`, `<button>`)**:
    - **`<input>`**: `type` 속성에 따라 다양한 형태의 입력 필드를 만듭니다.
        ...
        - **HTML5 유효성 검사 속성**: `pattern`(정규 표현식), `min`, `max`, `minlength`, `maxlength`, `step` 등 다양한 속성을 사용하여 클라이언트 측에서 입력값의 유효성을 검사할 수 있습니다. 이는 사용자 경험을 향상시키지만, **보안을 위해 서버 측 유효성 검사는 필수적**입니다.
