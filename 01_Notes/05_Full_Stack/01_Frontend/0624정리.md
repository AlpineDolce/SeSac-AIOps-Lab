...
- [2. CSS 선택자 (Selectors)](#2-css-선택자-selectors)
  ...
  - [2.3. 가상 클래스 (Pseudo-classes)](#23-가상-클래스-pseudo-classes)
  - [2.4. 가상 요소 (Pseudo-elements)](#24-가상-요소-pseudo-elements)
...
- [6. 성능, 접근성 및 개발 환경 고려사항](#6-성능-접근성-및-개발-환경-고려사항)
  ...
  - [6.11. CSS Custom Properties의 성능 및 유지보수 이점](#611-css-custom-properties의-성능-및-유지보수-이점)
...
- **`z-index`**: `position`이 `static`이 아닌 요소들의 쌓임 순서(stack order)를 결정합니다. 숫자가 클수록 위에 표시됩니다. `z-index`는 스태킹 컨텍스트(stacking context) 내에서만 작동하며, 스태킹 컨텍스트는 `position` 속성 외에도 `opacity`, `transform` 등 특정 CSS 속성에 의해 생성될 수 있습니다.
