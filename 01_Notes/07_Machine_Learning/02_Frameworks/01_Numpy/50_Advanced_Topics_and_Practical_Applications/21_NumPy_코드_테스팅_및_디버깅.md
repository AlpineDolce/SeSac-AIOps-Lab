...
- [3. NumPy 코드 디버깅 전략](#3-numpy-코드-디버깅-전략)
  ...
  - [3.3. 특수 값(NaN, Inf) 확인](#33-특수-값nan-inf-확인)
    - [3.3.1. `NaN` 생성 및 확인](#331-nan-생성-및-확인)
    - [3.3.2. `Inf` 생성 및 확인](#332-inf-생성-및-확인)
    - [3.3.3. `NaN`과 `Inf` 동시 확인](#333-nan과-inf-동시-확인)
  - [3.4. 작은 데이터로 문제 축소](#34-작은-데이터로-문제-축소)
...
컴퓨터의 이진수 표현 한계로 인해 부동소수점 연산은 미세한 오차를 포함할 수 있습니다. 이는 `0.1 + 0.2`가 정확히 `0.3`이 아닌 것처럼, 배열 간의 단순 비교(`==`)가 예상치 못한 `False`를 반환할 수 있습니다. 이러한 미세한 오차는 복잡한 계산 과정에서 누적되어 최종 결과에 큰 영향을 미칠 수 있으므로, 이를 고려한 테스트가 필수적입니다.
...
print(f"(arr1 + arr1) == arr2: {(arr1 + arr1) == arr2}") # [False False]
```

이러한 부동소수점 오차는 테스트뿐만 아니라 일반적인 코드에서도 배열 간의 비교나 조건문 작성 시 문제를 일으킬 수 있습니다. 따라서 `==` 연산자 대신 `np.isclose()`나 `np.allclose()`와 같은 함수를 사용하여 허용 오차 내에서 비교하는 것이 안전합니다.

*   **`np.isclose(a, b, rtol=1e-05, atol=1e-08)`**: 두 배열 `a`와 `b`의 각 요소가 허용 오차 내에서 '거의 같은지'를 불리언 배열로 반환합니다. `rtol`과 `atol`은 `numpy.testing.assert_allclose`와 동일한 의미를 가집니다.
*   **`np.allclose(a, b, rtol=1e-05, atol=1e-08)`**: 두 배열 `a`와 `b`의 모든 요소가 허용 오차 내에서 '거의 같은지'를 단일 불리언 값으로 반환합니다. `np.isclose()`의 결과를 `all()`로 묶는 것과 유사합니다.
...
# np.isclose를 사용한 비교
print(f"\nnp.isclose(a, b): {np.isclose(a, b)}") # True

arr1_sum = arr1 + arr1

# np.isclose를 배열에 적용
print(f"np.isclose(arr1_sum, arr2): {np.isclose(arr1_sum, arr2)}") # [True True]

# np.allclose를 사용한 배열 비교
print(f"np.allclose(arr1_sum, arr2): {np.allclose(arr1_sum, arr2)}") # True

# 오차가 큰 경우 (np.isclose/np.allclose도 False 반환)
result_large_error = np.array([0.300001])
desired_value = np.array([0.3])
print(f"\nnp.allclose(result_large_error, desired_value): {np.allclose(result_large_error, desired_value)}") # False (기본 허용 오차로는 부족)
print(f"np.allclose(result_large_error, desired_value, atol=1e-5): {np.allclose(result_large_error, desired_value, atol=1e-5)}") # True (atol 조정)
```
